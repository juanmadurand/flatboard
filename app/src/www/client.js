...
function isNotDeferred(item) {
  return !item.deferred;
}

function renderRedux(props) {
  return <ReduxAsyncConnect {...props} helpers={{client}} filter={isNotDeferred} />;
}

const component = (
  <Router render={renderRedux} history={history}>
    {getRoutes(store)}
  </Router>
);
